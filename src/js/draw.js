...
isWebview ? select('#android-warning').html("Note to Android users: if the search bar in our mobile app does not work for you, please visit our <a href='https://bit.ly/2Xp1I0Z' target='_blank'>guide in browser</a>.") : select('#android-warning').html("")
...
async function draw(){
	...
	async function searchWards(){
		...
		if (place == 'error finding place'){
			...
			errorDiv.innerHTML = '<p>We couldn\'t locate <strong>' + input.value + '</strong>. Please search a Miami area address.</p>';
			xtalkResize();
		} else {
			...
			if (houseShape | senShape == 'address not found in Florida'){
				...
				resultsDiv.style.display = 'none';
				errorDiv.style.display = 'block';
				errorDiv.innerHTML = '<p>We couldn\'t locate <strong>' + input.value + '</strong> in the Miami area. Please search a Miami area address.</p>';
				xtalkResize();
			} else {
	...
	function updateMap(shapes, coordinates){
		...
		hPartyName = shapes.house.properties.rep['Party'] == 'R' ? 'Republican' : 'Democrat', 
		sPartyName = shapes.sen.properties.sen['Party'] == 'R' ? 'Republican' : 'Democrat',
		...
		resultsDiv.innerHTML = '<div class="house-rep"><div class="pol-icon" style="border-color:'+hParty+'; background-image: url(\''+hPhoto+'\');"></div><div class="pol-text"><p class="office-signifier">State Representative</p>' + 
			'<p class="dist-name">'+hName+'</p>' +
			'<p class="rep-name">District ' + hDistrict + '</p>' +
			'<p>Party: '+hPartyName+'</p>' + 
			'<p><a href="'+hSite+'" target="_blank">Website <i class="fa fa-external-link" aria-hidden="true"></i></a></p>'+
			'</div></div>';

		resultsDiv.innerHTML += '<div class="house-rep"><div class="pol-icon" style="border-color:'+hParty+'; background-image: url(\''+sPhoto+'\');"></div><div class="pol-text"><p class="office-signifier">State Senator</p>' + 
			'<p class="dist-name">'+sName+'</p>' +
			'<p class="rep-name">District ' + sDistrict + '</p>' + 
			'<p>Party: '+hPartyName+'</p>' + 
			'<p><a href="'+hSite+'" target="_blank">Website <i class="fa fa-external-link" aria-hidden="true"></i></a></p>'+
			'</div></div>';
